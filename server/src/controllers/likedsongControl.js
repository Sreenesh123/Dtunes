...
import Song from "../models/likedsongModel.js";
...
const removeLikedSong = async (req, res) => {
  ...
  const { email } = req.params;
console.log("this is kirukan ",uri)
  try {
    const user = await User.findOne({ email });
...
const checkLikedSong = async (req, res) => {
  const { id } = req.params;
  const { email } = req.params;
console.log("this is the sreenesh email:",{email,id})
  try {
    const user = await User.findOne({ email });
    console.log("this is the sreenesh song user",user)
